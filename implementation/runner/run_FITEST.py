...
class caseStudy():
    def __init__(self):
        ...
        now = datetime.now()
        log_file = 'output/FITEST/' + str(now) + '_transfuser.log'
    ...
    def _evaluate(self,x):
        ...
        extra,DfC_min, DfV_max, DfP_max, DfM_max, DT_max, traffic_lights_max = get_values(abs_path+"RL-comps/all-data.txt")
        
        self.indx = self.indx+1
        logger.info(str(extra)+"$"+str(self.indx)+'#'+ str(DfC_min)+ ','+str( DfV_max)+','+str(DfP_max)+ ','
                    +str(DfM_max)+ ','+str( DT_max) +','+ str(traffic_lights_max))
        
        fo  = open (abs_path+"temp/"+str(self.indx)+'fitest'+str(datetime.now())+'.txt','w')
        fo.write(str(x))
        fo.close()
