...
def generate_random_population_using_LHS(size, lb, ub):
    from scipy.stats import qmc
    sampler = qmc.LatinHypercube(d=16)
    sample = sampler.random(n=5)
    ub[2] = 0.1
    pop_sampled = qmc.scale(sample, lb, ub)
    to_ret = []
    for ind in pop_sampled:
        to_add = [int(x) for x in ind]
        if to_add[0] != 3: # due to the ocl constraint
            to_add[15] = 0
...
def do_single_point_crossover(parent1, parent2):  # verify with  boss
...
def generate_off_spring(pop, objective_uncovered, lb,ub):
    ...
    while (len(population_to_return) < size):
        ...
        if probability_crossover <= 0.75:  # 75% probability
            parent1, parent2 = do_single_point_crossover(parent1, parent2)  # check with  boss
...
def generate_off_spring_mosaplus(pop, objective_uncovered, lb,ub):
    size = len(pop)
    population_to_return = []
    ...
    while (len(population_to_return) < size):
        ...
        if probability_crossover <= 0.75:  # 75% probability
            ...
            parent1, parent2 = do_simulated_binary_crossover(parent1, parent2, nc)  # check with  boss
...
def recombine(pop, objective_uncovered, lb, ub):
    ...
    if size == 1:
        ...
        while len(population_to_return) < size:
            ...
            probability_crossover = random.uniform(0, 1)
            if probability_crossover <= 0.60:  # 60% probability
                parent1, parent2 = do_simulated_binary_crossover(parent1, parent2)  # check with  boss
            child1, child2 = do_gaussain_mutation(parent1, parent2, lb, ub, (1 / len(parent1.get_candidate_values())))
