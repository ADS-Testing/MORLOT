...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def handle_actor_batch(batch):
        ...
        return actors
    @staticmethod
    def request_new_ped(model,spawn_point):
        mp= CarlaDataProvider._map


        wp1 = mp.get_waypoint(spawn_point.location, lane_type=(carla.LaneType.Sidewalk))

        from carla import Location, Transform,Rotation
        loc = Location(wp1.transform.location.x, wp1.transform.location.y, (wp1.transform.location.z + 0.8000))
        transform = Transform(loc, Rotation(0, 0, 0))

        return CarlaDataProvider.request_new_actor(model,transform)
