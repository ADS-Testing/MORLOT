...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        ego_vehicle,vif,ped = self._update_ego_vehicle()
        ...
        super(RouteScenario, self).__init__(name=config.name,
                                            ...
                                            criteria_enable=criteria_enable, vif = vif,ped = ped)
    ...
    def _update_ego_vehicle(self):
        ...
        vif =elevate_transform
        if int(scenario) == 2:
            vif.location.x = vif.location.x + 10

        else:
            vif.location.y = vif.location.y - 10

        vif = CarlaDataProvider.request_new_actor('vehicle.lincoln.mkz2017',
                                                          vif)

        ped = elevate_transform
        if int(scenario) == 2:
            ped.location.x = ped.location.x + 35
        else:
            ped.location.y = ped.location.y - 20

        pedestrian = CarlaDataProvider.request_new_ped('walker.pedestrian.0001',ped)

        return ego_vehicle,vif,pedestrian
